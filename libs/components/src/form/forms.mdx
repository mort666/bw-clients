...
If you are using these components in a new client, please ensure that the root scrollable element
(usually `body`) has the `ng-select-append-target` class. This tells the `ng-select` library, which
both components use under the hood, where to attach itself. When done correctly, the dropdown popup
should be positioned just underneath the dropdown trigger and should scroll with the page (not float
on top).
